...
def is_validation(user):
    ...
    if user['blood_group'] not in blood_types:
        error_check += 1
        error_log.append('blood_types')
    
    if '@' not in user['mail']:
        error_check += 1
        error_log.append('mail')
    
    if (2 > len(user['name'])) or (len(user['name']) > 30):
        error_check += 1
        error_log += 'name'
    
    if not user['website']:
        error_check += 1
        error_log.append('website')
...
#     global user_list
#     global err_num
#     for user_dict in user_data[0]:
#         print(user_dict)
#         temp_user = is_validation(user_dict)
#         print(temp_user)
...
def create_users(user_data):
    ...
    for user_dict in user_data:
        ...
        temp_user = is_validation(user_dict)
        # print(temp_user)
        if False == temp_user[0]:
            err_num += 1
            for key in temp_user[1]:
                user_dict[key] = None
    
    pprint.pprint(f"잘못된 데이터로 구성된 유저의 수는 {err_num}입니다.")

    return user_list

pprint.pprint(create_users(user_data))
# print(len(create_users(user_data)))
